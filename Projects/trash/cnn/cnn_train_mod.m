function [net, stats] = cnn_train_mod(net, train_db, val_db, get_batch_fun, save_in_folder, num_epochs)
...
opts.batchSize = 100;
opts.batchesPerEpoch = 1000;

evaluate_mode = false;

opts.expDir = save_in_folder;
opts.continue = true;
...
for epoch=start+1:opts.numEpochs
  ...
  params.getBatch = get_batch_fun ;

  if numel(params.gpus) <= 1
    [net, state] = processEpoch(net, state, params, train_db, 'train') ;
    [net, state] = processEpoch(net, state, params, val_db, 'val') ;
    if ~evaluate_mode
...
% -------------------------------------------------------------------------
function [net, state] = processEpoch(net, state, params, imdb, mode)
% -------------------------------------------------------------------------
...
subset = 1:params.batchSize*params.batchesPerEpoch ;
num = 0 ;
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    [im, labels] = params.getBatch(imdb, batchSize) ;
    ...
    if params.prefetch
      ...
      params.getBatch(imdb, batchSize) ;
    end
