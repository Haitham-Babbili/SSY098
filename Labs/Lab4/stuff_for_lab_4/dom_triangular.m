function dom_triangular(pts, pts_tilda)
...
[pts_target, desc_target] = extractSIFT(mean(pts_tilda, 3));
...
% Creat RANSAC to identify the 3D point

N=length(u); % calculate camera matrix number


Uhat = zeros(3, N); % creat eampty 3D point matrix


nbr_inliers = zeros(N, 1); % creat empty vector matix  for inlier 3D points


for i = 1 : N
    
    % mearg the know 2 image points
    us = [u(:, i) u_tilde(:, i)];
...
% Clean and remove then refind new point after that save it  
